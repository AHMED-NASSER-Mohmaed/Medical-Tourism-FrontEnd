...
import { CarRentalProvider, UserStatus, PaginatedResponse } from '../../../models/super-admin.model';
import { finalize } from 'rxjs/operators';
import { faCar, faUserCheck, faUserClock, faTimes } from '@fortawesome/free-solid-svg-icons';
import { Router } from '@angular/router';

@Component({
  selector: 'app-car-rentals-accounts',
  standalone: false,
  templateUrl: './car-rentals-accounts.component.html',
  styleUrl: './car-rentals-accounts.component.css'
})
export class CarRentalsAccountsComponent implements OnInit {
  accounts: CarRentalProvider[] = [];
  pagination = { page: 1, pageSize: 10, totalCount: 0, totalPages: 0 };
  isLoading = false;
  searchTerm = '';
  errorMessage = '';
  showAccountModal = false;
  selectedAccount: CarRentalProvider | null = null;
  ...
  loadAccounts(): void {
    ...
    this.superAdminService.getCarRentalProviders(this.pagination.page, this.pagination.pageSize, filters)
      .pipe(finalize(() => this.isLoading = false))
      .subscribe({
        next: (response: PaginatedResponse<CarRentalProvider>) => {
        ...
        error: (err) => {
          this.errorMessage = err.userMessage || 'Failed to load car rentals';
        }
      });
  }
  ...
  async viewAccount(account: CarRentalProvider): Promise<void> {
