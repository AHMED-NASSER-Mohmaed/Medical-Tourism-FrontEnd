...
import { HospitalProvider, UserStatus, PaginatedResponse } from '../../../models/super-admin.model';
import { finalize } from 'rxjs/operators';
import { faHospital, faUserCheck, faUserClock, faTimes } from '@fortawesome/free-solid-svg-icons';
import { Router } from '@angular/router';

@Component({
  selector: 'app-hospitals-accounts',
  standalone: false,
  templateUrl: './hospitals-accounts.component.html',
  styleUrl: './hospitals-accounts.component.css'
})
export class HospitalsAccountsComponent implements OnInit {
  accounts: HospitalProvider[] = [];
  pagination = { page: 1, pageSize: 10, totalCount: 0, totalPages: 0 };
  isLoading = false;
  searchTerm = '';
  errorMessage = '';
  showAccountModal = false;
  selectedAccount: HospitalProvider | null = null;
  ...
  loadAccounts(): void {
    ...
    this.superAdminService.getHospitalProviders(this.pagination.page, this.pagination.pageSize, filters)
      ...
      .subscribe({
        next: (response: PaginatedResponse<HospitalProvider>) => {
        ...
        error: (err) => {
          this.errorMessage = err.userMessage || 'Failed to load hospitals';
        }
      });
  }
